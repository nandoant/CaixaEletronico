...
export const SaquePage: React.FC = () => {
  ...
  const [opcaoSelecionada, setOpcaoSelecionada] = useState<SaqueOpcao | null>(null);
  ...
  const handleSolicitarOpcoes = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao buscar opções de saque');
    } finally {
      setLoading(false);
    }
  };
                ...
                <Grid item xs={12} md={6}>
                  <Box sx={{ display: 'flex', alignItems: 'center', height: '100%' }}>
                    <Typography variant="h6" color="primary">
                      {valorNumerico > 0 ? formatarValor(valorNumerico) : 'R$ 0,00'}
                    </Typography>
                  </Box>
                </Grid>
              </Grid>
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                ...
                <Typography variant="h6">
                  Escolha a combinação de cédulas para {formatarValor(valorNumerico)}
                </Typography>
              </Box>
                    ...
                    >
                      <Stack direction="row" justifyContent="space-between" alignItems="center">
                        <Box>
                          ...
                          <Chip 
                            label={`${opcao.quantidadeTotalDeNotas} nota${opcao.quantidadeTotalDeNotas > 1 ? 's' : ''}`}
                            size="small"
                            color="primary"
                          />
                        </Box>
            ...
            <CardContent>
              ...
              <TableContainer component={Paper} variant="outlined">
                <Table>
                  <TableBody>
                    <TableRow>
                      <TableCell><strong>Valor do Saque</strong></TableCell>
                      <TableCell>{formatarValor(valorNumerico)}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Combinação Escolhida</strong></TableCell>
                      <TableCell>{opcaoSelecionada?.descricaoLegivel}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Total de Notas</strong></TableCell>
                      <TableCell>
                        {opcaoSelecionada?.quantidadeTotalDeNotas} nota{opcaoSelecionada?.quantidadeTotalDeNotas !== 1 ? 's' : ''}
                      </TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              ...
              <Alert severity="warning" sx={{ mt: 3 }}>
                <Typography variant="body2">
                  <strong>Atenção:</strong> Após confirmar o saque, a operação não poderá ser cancelada.
            ...
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <TableContainer component={Paper} variant="outlined" sx={{ mt: 3 }}>
                <Table>
                  <TableBody>
                    <TableRow>
                      <TableCell><strong>Valor Sacado</strong></TableCell>
                      <TableCell>{formatarValor(valorNumerico)}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Combinação</strong></TableCell>
                      <TableCell>{opcaoSelecionada?.descricaoLegivel}</TableCell>
                    </TableRow>
                    <TableRow>
